function [grd_Fe,gd2_Fe] = GrdForceElm_body_hvi_FD(i_tip,x_crk,...
    dx_elm,x_elm,dx_sub,x_sub,l_sub,N_sub,dNdX_sub,W_sub,f) % ,order

%--------------------------------------------------------------------------

% (!) for partial-rotation elements
% (!) this script is (almost) the same as:
%       GrdForceElm_resid_hvi_FD, GrdStiffElm_hvi_FD
...
for i = 1:order+1 % (order+1 == n. samples)
    ...
    [N_elm,dNdX_elm] = ShapesStd_omg(n_gsp,X_gsp);
    ...
    H_gpD = reshape(repmat(H_gsp(:)',2,1),[],1); % (for derivs.)
    ...
    for j = 1:n_nod
        N_elm(:,n_nod+j) = N_elm(:,j).*(H_gsp-H_nod(j));
        dNdX_elm(:,n_nod+j) = dNdX_elm(:,j).*(H_gpD-H_nod(j));
    end
    
    Fe{i} = ForceElm_body(x_elm,N_elm,dNdX_elm,W_elm,f);
    % Fe{i} = ForceElm_resid(x_elm,N_eln,dNdX_elm,W_elm,F);
