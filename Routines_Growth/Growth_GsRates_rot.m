...
for i = vElMov(:)' % (el. on the periphery)
    ...
    if wBdLod
        ...
        [grd_Fe,gd2_Fe] = GrdForceElm_body('rot',dx_elm,x_elm,...
            mGsStd_omgShp,mGsStd_omgDrv,vGsStd_omgWgt,vBdLod);
 
        Gs_tht = Gs_tht + u'*grd_Fe;
        Hs_tht = Hs_tht + u'*gd2_Fe;
        grd_Fi(p)= grd_Fi(p)-grd_Fe;
        
    end
    
    if wPrLod
        
        [grd_Fe,gd2_Fe] = GrdForceElm_resid('rot',dx_elm,...
            x_elm,mGsStd_omgDrv,vGsStd_omgWgt,mPrLod(:,i));
...
if length(cLNodE{i})==nLNodS % assert that only Heaviside enrichment
    ...
    if wCkLod % (crack pressure)
        ...
        % too tedious to do analytically; use 4th order finite difference
    ...
    if wBdLod % (body load)
        ...
        % too tedious to do analytically; use 4th order finite difference
        [grd_Fe,gd2_Fe] = GrdForceElm_body_hvi_FD(i_tip,x_crk,dx_elm,x_elm,...
            dx_sub,x_sub,l_sub,mGsHvi_subShp,mGsHvi_subDrv,vGsHvi_subWgt,vBdLod);
    ...
    if wPrLod % (pre-load or residual load)
        
        % too tedious to do analytically; use 4th order finite difference
        [grd_Fe,gd2_Fe] = GrdForceElm_body_hvi_FD(i_tip,x_crk,dx_elm,x_elm,...
            dx_sub,x_sub,l_sub,mGsHvi_subShp,mGsHvi_subDrv,vGsHvi_subWgt,mPrLod(:,i));
...
for i = vElShf(:)'
    ...
    Gs_tht = Gs_tht - u'*Ke*uT;
    Hs_tht = Hs_tht + u'*Ke*u-uT'*Ke*uT;
    ...
    if wCkLod % (crack pressure)
        ...
        for j = 1:size(cLEnDt{i},2)
            ...
            elseif cLEnDt{i}(2,j) == 0 % Hvi. enr.
                ...
                [grd_Fe(p_frc),gd2_Fe(p_frc)] = GrdForceElm_crack_hvi(i_tip,...
                    x_inc,x_elm,mGsHvi_gamShp,vGsHvi_gamWgt,mCkLod(i_crk,:));
...
%     if wBdLod % (body load)
%         
%         % if vBdLod = const, then grd_Fe = 0, gd2_Fe = 0, because
%         % of a rigid body translation/rotation for which d_det(J) = 0
%         
%     end
    
    if wPrLod % (pre-load or residual load)
        
        % even if vPrLod = const, grd_Fe ~= 0 and gd2_Fe ~= 0, because
        % d_inv(J) ~= 0 due to the rigid rotation (though d_det(J) = 0)
        
        dx_elm=[x_inc(1,2)-x_elm(:,2),...
            x_elm(:,1)-x_inc(1,1)];
        
%         [grd_Fe,gd2_Fe] = GrdForceElm_resid('rot',dx_elm,x_elm,...
%             [cGsEnr_omgDvS{i},cGsEnr_omgDvE{i}],cGsEnr_omgWgt{i},mPrLod(:,i));
        
        % "GrdForceElm_resid_rot" is a simplification of "GrdForceElm_resid", by 
        % taking advantage of the fact that the element undergoes a pure rotation 
        % (no distortion), which can be shown to lead to: d_det(J) = 0, and a 
        % simpler and more efficient evaluation of d[d/dx,d/dy]

        [grd_Fe,gd2_Fe] = GrdForceElm_resid_rot(dx_elm,x_elm,...
            [cGsEnr_omgDvS{i},cGsEnr_omgDvE{i}],cGsEnr_omgWgt{i},mPrLod(:,i));
...
clear tmp q p p_nul u uT grd_Ke gd2_Ke grd_Fi grd_Fg
