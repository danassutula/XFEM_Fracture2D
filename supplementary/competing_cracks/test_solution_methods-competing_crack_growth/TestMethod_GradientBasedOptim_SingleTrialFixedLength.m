...
% This is faster than brute-force optimisation, i.e. testing each crack tip 
% extension separately. In brute-force you have 'n_tips'-evaluations of 
% the total energy, whereas in the modified gradient-based approach, you 
% have at worts: n_tips/2 evaluations, which happens when all tips have 
% very similar crack tip energy release rates.

% Goal: test if 'ActiveSetOptim_gradient_discrete' works with one trial
% solution which is uniformly extending all crack tips by da_inc and 
% then, according to the algorithm, cutting out the less energetically
% favourable extensions. The algorithm needs to be robust for any number 
% of crack tips and for the following cases:
...
%
% The algorithm will not be robust for the case of:
...
% n_try = 1; % consider a single trial crack extension: uniform extension

while sum(da_h) < da_tot % number of crack increments
    
    Gs_h = Gs+Hs*da_h; % current energy release rates at crack tips
    da = ones(n_tips,1)*da_inc; % trial solution: extend all tips uniformly
    
    da = CrackGrowthSol_GradientBasedOptim_fixedLength(Gs_h,Hs,da);
    % final solution, sum(da) == da_inc
