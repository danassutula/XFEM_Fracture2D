...
with_Layered  = 1; % 1 - layered domain , 0 - monolithic

if with_Layered == 1
    with_MeshRead = 0; % unexpected outcome otherwise
end

mesh_ElemType = 'T3'; % element types: 'T3' or 'Q4'
mesh_EnriSize = 'normal'; % tip enrichment radius: 'small', 'normal' or 'big'



% Crack growth criteria:
kind_LawDir = 'energy'; % maxhoop; energy; symmetry;
kind_LawCrt = 'energy'; % tension; energy; J-int; eliptic; Hayashi; Nuismer;
kind_GrwCrt = 'custom'; % maximum; symmetric; critical; all; custom;

if strcmpi(kind_GrwCrt,'custom')
    % lowers threshold for growth (useful if with_GLwInc == 1)
    with_GrwCrt_inf = 0.98; % (as fraction of maximum)
end
...
with_AdpEnr = 1; % adjust tip enrichment radius based on local mesh size
...
with_RfnInc = 1; % addaptive re-meshing: kink dependent
with_RfnXrs = 1; % addaptive re-meshing: pre-intersection
...
% Supplementary:
with_JIntegral = 0; % crack tip energy release rate: 'Ji' 
