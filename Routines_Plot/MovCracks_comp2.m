...
% load ck. coord.
load([path_exF,'var_Crack',num2str(280,'_%05d')]);
cCkCrd = sCrack.cCkCrd;
...
%     
%     for j = 1:length(cCkCrd)
%         a3 = plot(cCkCrd{j}(:,1),cCkCrd{j}(:,2),'-g','linewidth',1);
%     end
    ...
    xlim_zm1 = xlim_zm1 + speed;
    set(gca,'xlim',xlim_zm1,'ylim',ylim_zm1)
    legend([a1f(1),a1(1),a2(1)],'max-hoop    (  > 2mln. DOF )','max-hoop    (\approx0.5mln. DOF)','energy min. (\approx0.5mln. DOF)')
