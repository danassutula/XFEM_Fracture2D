function [coord,lnods,bound,elphz] = MeshLayers_t3(xlim,Y,N,h0)
...
if N(2) > N(1) % with step-down
    ...
    % blending mesh depth
    dy = dY/round(dY/(hx*sqrt(3)*0.5))*f;
    
    % layer depth
    dY = dY-dy;
    
    % mesh layer
    [coord,lnods] = MeshRect_t3(xlim,[Y(1),Y(2)-dy],hx); % nx+1,round(dY/hx)+1);
    
    % mesh layer blending region
    [c,l] = StepDown_t3(xlim,[Y(2)-dy,Y(2)],nx+1,N(2)-N(1)+1);
    ...
    [coord,lnods] = MeshRect_t3(xlim,[Y(1),Y(2)],hx); % nx+1,round(dY/hx)+1);
...
for i = 2:length(N)-1 % for ith layer
    ...
    if N(i+1) > N(i) % with step-down
        ...
        % mesh layer
        [c,l] = MeshRect_t3(xlim,[Y(i),Y(i+1)-dy],hx); % nx+1,round(dY/hx)+1);
        ...
        % mesh layer
        [c,l] = MeshRect_t3(xlim,[Y(i),Y(i+1)],hx); % nx+1,round(dY/hx)+1);
